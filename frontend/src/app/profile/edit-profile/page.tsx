...
const formSchema = z
  .object({
    ...
    industry: z
      .string()
      .min(1, { message: "Industry must be at least 1 character." })
      .optional(),

    yearsExperience: z.coerce.number()
      .min(0, { message: "Years of experience must be at least 1 digit." })
      .optional(),
...
function EditProfileContent() {
  ...
  useEffect(() => {
    const subscription = methods.watch((value, { name }) => {
      ...
      if (name === "role" && value.role !== user.role) {
        newModified.add("role");
      } else if (name === "role") {
        newModified.delete("role");
      }

      if (name === "industry" && value.industry !== user.industry) {
        newModified.add("industry");
      } else if (name === "industry") {
        newModified.delete("industry");
      }

      if (name === "yearsExperience" && value.yearsExperience !== user.years_experience) {
        newModified.add("yearsExperience");
      } else if (name === "yearsExperience") {
        newModified.delete("yearsExperience");
      }
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (values.industry && values.industry !== user?.industry) {
      payload.industry = values.industry;
    }
    if (values.yearsExperience && values.yearsExperience !== user?.years_experience) {
      payload.years_experience = values.yearsExperience;
    }
    if (values.password) {
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        ...
                        aria-label="Industry"
                        aria-invalid={!!methods.formState.errors.industry}
                        {...field}
                      />
                    </FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Years of Experience
                      {modifiedFields.has("yearsExperience") && (
                        <span className="ml-2 text-xs text-blue-600 dark:text-blue-400">(modified)</span>
                      )}
                    </FormLabel>
                    <FormControl>
                      <Input
                        type="number" min={0}
                        placeholder={user.years_experience?.toString() ?? "0"}
                        aria-label="Years of Experience"
                        aria-invalid={!!methods.formState.errors.yearsExperience}
                        {...field}
                      />
                    </FormControl>
                    {methods.formState.errors.yearsExperience && (
                      <p className="mt-1 text-sm text-destructive" role="alert">
                        {methods.formState.errors.yearsExperience.message}
                      </p>
                    )}
                  </FormItem>
                )}
